...
import { RobustYouTubeTranscriptExtractor, YouTubeTranscriptErrorHandler, YouTubeTranscriptError, YouTubeTranscriptErrorType, DEFAULT_RETRY_CONFIG, ConsoleLogger, SimpleMetricsCollector } from './transcriptExtractor.js';
// Create a mock transcript fetcher using dependency injection
const mockFetchTranscript = jest.fn();
const mockTranscriptFetcher = {
...
describe('YouTubeTranscriptError', () => {
    it('should create error with correct properties', () => {
        const error = new YouTubeTranscriptError(YouTubeTranscriptErrorType.TRANSCRIPT_DISABLED, 'Test message', 'video123', 'Original error');
...
describe('RobustYouTubeTranscriptExtractor', () => {
    ...
    beforeEach(() => {
        ...
        extractor = new RobustYouTubeTranscriptExtractor(testRetryConfig, mockLogger, mockMetrics, mockTranscriptFetcher);
        // Reset all mocks
        jest.clearAllMocks();
    ...
    describe('extractTranscript - Error Handling', () => {
        it('should handle transcript disabled error', async () => {
            mockFetchTranscript.mockRejectedValue(new Error('Transcript is disabled for this video'));
            const result = await extractor.extractTranscript('test123');
            expect(result.success).toBe(false);
            expect(result.error?.type).toBe(YouTubeTranscriptErrorType.TRANSCRIPT_DISABLED);
        ...
        errorScenarios.forEach(scenario => {
            it(`should handle ${scenario.name} correctly`, async () => {
                mockFetchTranscript.mockRejectedValue(new Error(scenario.errorMessage));
                const result = await extractor.extractTranscript('test123');
                expect(result.success).toBe(false);
    ...
    describe('Custom Configuration', () => {
        it('should use custom retry configuration', async () => {
            ...
            const customExtractor = new RobustYouTubeTranscriptExtractor(customConfig, mockLogger, mockMetrics, mockTranscriptFetcher);
            mockFetchTranscript.mockRejectedValue(new Error('Network connection failed'));
...
//# sourceMappingURL=transcriptExtractor.spec.js.map